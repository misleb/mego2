...
func incHandler(c *gin.Context, param types.IntRequest, _ *store.DB) {
	c.JSON(200, types.IntResponse{Result: param.Value + 1})
}

func loginHandler(c *gin.Context, param types.LoginRequest, d *store.DB) {
	user, err := d.GetUserByEmailAndPassword(c.Request.Context(), param.Username, param.Password)
	if err != nil {
...
func googleAuthHandler(c *gin.Context, param types.GoogleAuthRequest, d *store.DB) {
	...
	err = d.FindOrCreateUserByEmail(c.Request.Context(), user)
	if err != nil {
...
func updateSelfHandler(c *gin.Context, param types.UpdateSelfRequest, d *store.DB) {
	...
	if param.NewPassword != "" {
		if user.IsNewExternal {
			...
			if _, err := d.GetUserByEmailAndPassword(c.Request.Context(), user.Email, param.OldPassword); err != nil {
